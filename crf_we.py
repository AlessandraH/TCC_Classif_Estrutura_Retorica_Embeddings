...
def abstract2features(abstract, we, pos, c):
    return [sent2features(c+i, we[c+i], pos) for i in range(len(abstract))]
...
def classificador():
    ...
    for corpus in corpora:
        ...
        c = 0
        for a in abstracts:
            X_crf.append(abstract2features(a, X_sentences_we, X_pos, c))
            c += len(a)
